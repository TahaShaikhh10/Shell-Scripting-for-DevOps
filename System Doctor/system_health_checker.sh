#!/bin/bash
# Script: system_health_pro.sh

# 1. Setup
REPORT_DIR="/var/log/health_reports"
DATE=$(date +%Y-%m-%d_%H-%M-%S)
REPORT_FILE="$REPORT_DIR/health_report_$DATE.html"
SLACK_WEBHOOK_URL="https://hooks.slack.com/services/XXXXX/XXXXX/XXXXX" # replace with your own
...
# 10. Send alert to Slack
MESSAGE="📊 New System Health Report generated: $REPORT_FILE"
curl -X POST -H 'Content-type: application/json' \
    --data "{\"text\":\"$MESSAGE\"}" \
    $SLACK_WEBHOOK_URL

echo "✅ Report saved at $REPORT_FILE and Slack alert sent."
